...
首页采用对话式工作区：输入、图片附件、模型选择和发送按钮固定在同一底部编辑器。普通问题可以连续对话；涉及具体参数时，服务端仅能调用材料概况、目标填写、相似案例、知识检索和参数推荐五项已注册工具。推荐仍返回一组：优先使用满足全部要求的历史实测案例，否则由训练集内分组验证选择模型。生成新参数需要明确设备步长，未定义步长的字段保持历史设置；不把已知不合格历史参数重新包装为模型推荐。

`POST /api/agent/conversations/{id}/turns` 保存用户消息、助手回复、工具事件、当前目标和可选推荐引用；同一请求标识会复用已完成的外部调用结果。每轮最多六次工具调用，图片只在本轮发送给选择的支持图片模型，不写入知识库。旧 `/messages` 保留为结构化表单兼容入口，旧 `{text,result}` 历史可继续读取。
